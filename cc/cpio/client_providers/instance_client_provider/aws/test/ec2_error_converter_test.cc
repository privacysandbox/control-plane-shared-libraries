...
#include "cpio/client_providers/instance_client_provider/aws/src/ec2_error_converter.h"
...
TEST(EC2ErrorConverter, SucceededToConvertHandledEC2Errors) {
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::VALIDATION),
            FailureExecutionResult(SC_AWS_VALIDATION_FAILED));
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::ACCESS_DENIED),
            FailureExecutionResult(SC_AWS_INVALID_CREDENTIALS));
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(
                EC2Errors::INVALID_PARAMETER_COMBINATION),
            FailureExecutionResult(SC_AWS_INVALID_REQUEST));
  EXPECT_EQ(
      EC2ErrorConverter::ConvertEC2Error(EC2Errors::INVALID_QUERY_PARAMETER),
      FailureExecutionResult(SC_AWS_INVALID_REQUEST));
  EXPECT_EQ(
      EC2ErrorConverter::ConvertEC2Error(EC2Errors::INVALID_PARAMETER_VALUE),
      FailureExecutionResult(SC_AWS_INVALID_REQUEST));
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::INTERNAL_FAILURE),
  ...
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::NETWORK_CONNECTION),
            FailureExecutionResult(SC_AWS_SERVICE_UNAVAILABLE));
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::THROTTLING),
...
TEST(EC2ErrorConverter, SucceededToConvertNonHandledEC2Errors) {
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(
                EC2Errors::INVALID_GROUP_ID__MALFORMED),
            FailureExecutionResult(SC_AWS_INTERNAL_SERVICE_ERROR));
  EXPECT_EQ(EC2ErrorConverter::ConvertEC2Error(EC2Errors::DRY_RUN_OPERATION),
            FailureExecutionResult(SC_AWS_INTERNAL_SERVICE_ERROR));
  EXPECT_EQ(
      EC2ErrorConverter::ConvertEC2Error(EC2Errors::OPERATION_NOT_PERMITTED),
      FailureExecutionResult(SC_AWS_INTERNAL_SERVICE_ERROR));
}
}  // namespace google::scp::cpio::client_providers::test
