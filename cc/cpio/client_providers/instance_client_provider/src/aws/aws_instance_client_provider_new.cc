...
#include "aws_instance_client_provider_new.h"
...
namespace {
constexpr char kAwsInstanceClientProviderNew[] = "AwsInstanceClientProvider";
...
void AwsInstanceClientProviderNew::OnGetSessionTokenCallback(
  ...
  if (!get_token_context.result.Successful()) {
    ...
    FinishContext(get_token_context.result, get_resource_name_context,
                  cpu_async_executor_);
    return;
  }
  ...
  if (!execution_result.Successful()) {
    ERROR_CONTEXT(kAwsInstanceClientProviderNew, get_resource_name_context,
                  ...
                  "resource id.");
    FinishContext(execution_result, get_resource_name_context,
                  cpu_async_executor_);
    return;
  }
}
...
void AwsInstanceClientProviderNew::OnGetInstanceResourceNameCallback(
  ...
  if (!http_client_context.result.Successful()) {
    ...
    FinishContext(http_client_context.result, get_resource_name_context,
                  cpu_async_executor_);
    return;
  }
...
void AwsInstanceClientProviderNew::OnDescribeInstancesAsyncCallback(
  ...
  if (!outcome.IsSuccess()) {
    ...
    get_details_context.result = EC2ErrorConverter::ConvertEC2Error(
  ...
  get_details_context.result = SuccessExecutionResult();
  get_details_context.Finish();
}
...
void AwsInstanceClientProviderNew::OnDescribeTagsAsyncCallback(
  ...
  if (!outcome.IsSuccess()) {
    const auto& error_type = outcome.GetError().GetErrorType();
    get_tags_context.result = EC2ErrorConverter::ConvertEC2Error(
        error_type, outcome.GetError().GetMessage());
    ERROR_CONTEXT(kAwsInstanceClientProviderNew, get_tags_context,
                  get_tags_context.result,
...
ExecutionResult AwsInstanceClientProviderNew::GetCurrentInstanceId(
    string& instance_id) noexcept {
  // Not implemented.
  return FailureExecutionResult(SC_UNKNOWN);
}

ExecutionResult AwsInstanceClientProviderNew::GetCurrentInstanceRegion(
...
ExecutionResult
AwsInstanceClientProviderNew::GetCurrentInstancePrivateIpv4Address(
    string& instance_private_ipv4_address) noexcept {
  // Not implemented.
  return FailureExecutionResult(SC_UNKNOWN);
}

ExecutionResult AwsInstanceClientProviderNew::GetTagsOfInstance(
