...
static constexpr char kCreationTime[] = "creationTime";
...
ExecutionResult PrivateKeyFetchingClientUtils::ParsePrivateKey(
  ...
  result = ParseJsonValue(json_response, kExpirationTime, expiration_time);
  if (!result.Successful()) {
    return FailureExecutionResult(
        SC_PRIVATE_KEY_FETCHER_PROVIDER_EXPIRATION_TIME_NOT_FOUND);
  }
  response.expiration_time_in_ms = std::stol(expiration_time);

  string creation_time;
  result = ParseJsonValue(json_response, kCreationTime, creation_time);
  if (!result.Successful()) {
    return FailureExecutionResult(
        SC_PRIVATE_KEY_FETCHER_PROVIDER_CREATION_TIME_NOT_FOUND);
  }
  response.creation_time_in_ms = std::stol(creation_time);
