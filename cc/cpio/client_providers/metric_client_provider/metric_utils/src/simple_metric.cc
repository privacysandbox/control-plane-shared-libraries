...
void SimpleMetric::RunMetricPush(const shared_ptr<RecordMetricsProtoRequest>
  ...
  AsyncContext<RecordMetricsProtoRequest, RecordMetricsProtoResponse>
      record_metric_context(
          ...
          [&](AsyncContext<RecordMetricsProtoRequest,
                           RecordMetricsProtoResponse>& outcome) {
            if (!outcome.result.Successful()) {
              // TODO: Create an alert or reschedule
            }
          });
  auto execution_result = metric_client_->RecordMetrics(record_metric_context);
...
void SimpleMetric::Push(const shared_ptr<MetricValue>& metric_value,
  ...
  MetricUtils::GetRecordMetricsProtoRequest(record_metric_request, metric_info_,
