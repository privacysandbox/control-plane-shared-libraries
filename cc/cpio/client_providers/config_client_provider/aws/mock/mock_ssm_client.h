...
class MockSSMClient : public Aws::SSM::SSMClient {
 public:
  Aws::SSM::Model::GetParametersOutcome GetParameters(
      const Aws::SSM::Model::GetParametersRequest& request) const override {
    if (IsEqual(request.GetNames(), get_parameters_request_mock.GetNames())) {
      return get_parameters_outcome_mock;
    }
    return Aws::SSM::Model::GetParametersOutcome();
  }
