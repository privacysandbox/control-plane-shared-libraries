...
Worker::Worker(RoleId role_id, const Config& config)
  ...
  config.GetJsEngineResourceConstraints(v8_resource_constraints);
  std::vector<std::shared_ptr<FunctionBindingObjectBase>> function_bindings;
  config.GetFunctionBindings(function_bindings);
  execution_manager_ =
      make_unique<ExecutionManager>(v8_resource_constraints, function_bindings);
}
